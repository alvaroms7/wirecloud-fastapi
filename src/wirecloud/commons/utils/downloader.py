...
async def download_http_content(url: str) -> bytes:
    parsed_url = urlparse(url)
    if parsed_url.scheme not in ('http', 'https'):
        raise ValueError(_('Invalid schema: %(schema)s') % {"schema": parsed_url.scheme})

    headers = {
        'User-Agent': 'Mozilla/5.0 (%(system)s %(machine)s;U) Wirecloud/%(wirecloud_version)s python-httpx/%(httpx_version)s' % VERSIONS,
        'Accept': '*/*',
